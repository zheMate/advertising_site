<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800 dark:text-gray-200 leading-tight">
            {{ __('Редактирование объявления - ') }} "{{ $promo->title }}"        
        </h2>
    </x-slot>
                ...
                <div class="p-6 text-gray-900 dark:text-gray-100">
                    ...
                    <form action="{{ route('dashboard.promo.update', $promo->id) }}" method="POST">
                        ...
                        <div class="mb-6">
                            ...
                            <input type="text" id="title" name="title" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500" placeholder="Например: Часы..." value="{{ $promo->title }}">
                            @error('title')
                        ...
                        <div class="mb-6">
                            ...
                            <select id="category" name="category_id" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500">
                                ...
                                <option value="{{ $category->id }}" {{ $category->id == $promo->category_id ? ' selected' : '' }}>{{ $category->title }}</option>
                                @endforeach
                            </select>
                            @error('category_id')
                            <div class="text-sm text-red-600 dark:text-red-400 space-y-1 mt-2">{{ $message }}</div>
                        ...
                        dark:focus:ring-offset-gray-800 transition ease-in-out duration-150 " value="Обновить">
